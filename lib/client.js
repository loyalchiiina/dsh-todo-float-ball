window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var historyOpen = false; // Plan-A3: completed-history fold collapsed by default
    var historyOpenOpen = false; // Plan-A3.1: non-completed-history fold collapsed by default
    // Plan-A8: per-pinned-session fold states (same UX as the current view)
    var pinnedHistOpen = {};     // sid -> completed-history fold open?
    var pinnedOpenHistOpen = {}; // sid -> abandoned-history fold open?
    ...
    function snapshotList() {
      ...
      var b = bySession[normSid(currentSid)];
      var snapLen = (b && typeof b.snapLen === "number") ? Math.min(b.snapLen, list.length) : list.length;
      return list.slice(0, snapLen);
    }
    ...
    function renderList() {
      ...
      if (!list.length) {
        ...
        // Plan-A3: split into the fresh snapshot (fully visible) and the
        // accumulated history (collapsed by default, completed-only).
        var snapLen = 0;
        var b = bySession[normSid(currentSid)];
        if (b && typeof b.snapLen === "number") snapLen = Math.min(b.snapLen, list.length);
        var snapRows = list.slice(0, snapLen);
        var histRows = list.slice(snapLen);
        var histDone = [];
        ...
        if (histDone.length) {
          html += '<li class="tfb-histoggle" data-act="toggleHistory" title="展开/收起历史任务">▾ 已完成历史 ' + histDone.length + ' 条（点击' + (historyOpen ? "收起" : "展开") + '）' +
            '<button class="tfb-btn" data-act="clearHistory" type="button" title="一键隐藏本区全部条目（✕ 同款，仅显示层）">🧹</button></li>';
          if (historyOpen) html += listHtml(histDone);
        }
        // Plan-A3.1: non-completed history in a SECOND fold (default closed)
        if (histOpen_.length) {
          html += '<li class="tfb-histoggle" data-act="toggleHistoryOpen" title="展开/收起被抛弃的历史任务">▸ 被抛弃的历史任务 ' + histOpen_.length + ' 条（点击' + (historyOpenOpen ? "收起" : "展开") + '）' +
            '<button class="tfb-btn" data-act="clearHistoryOpen" type="button" title="一键隐藏本区全部条目（✕ 同款，仅显示层）">🧹</button></li>';
          if (historyOpenOpen) html += listHtml(histOpen_);
        }
        if (anyHist) {
          html += '<li class="tfb-histoggle" data-act="clearAll" title="一键隐藏两个历史区全部条目">🧹 一键清理全部历史（' + anyHist + ' 条）</li>';
        }
        ...
        var hidCount = 0;
        var hidMap = hiddenFor(currentSid);
        for (var hk2 in hidMap) { if (hidMap[hk2]) hidCount++; }
        if (hidCount > 0) {
          html += '<li class="tfb-histoggle" data-act="restoreAll" title="取消本会话全部隐藏（✕/🧹 的反向操作）">♻️ 一键恢复全部隐藏的历史（' + hidCount + ' 条）</li>';
        }
    ...
    function renderPinned() {
      ...
      for (var i = 0; i < pinned.length; i++) {
        ...
        if (open) {
          if (list && list.length) {
            // Plan-A8: pinned sessions get the SAME density treatment as the
            // current view — snapshot rows flat, then per-session history
            // folds (completed / abandoned), each with its own open state
            // and a 🧹 batch-clear. Fold state is remembered per session.
            var pb = bySession[normSid(sid)];
            var pSnapLen = (pb && typeof pb.snapLen === "number") ? Math.min(pb.snapLen, list.length) : list.length;
            var pSnap = list.slice(0, pSnapLen);
            var pHist = list.slice(pSnapLen);
            var pDone = [], pOpen_ = [];
            for (var ph = 0; ph < pHist.length; ph++) {
              if (pHist[ph].status === "completed") pDone.push(pHist[ph]);
              else pOpen_.push(pHist[ph]);
            }
            var ph_html = listHtml(pSnap);
            if (pDone.length) {
              ph_html += '<li class="tfb-histoggle" data-act="toggleHistPinned" data-sid="' + esc(sid) + '" title="展开/收起历史任务">▾ 已完成历史 ' + pDone.length + ' 条（点击' + (pinnedHistOpen[sid] ? "收起" : "展开") + '）' +
                '<button class="tfb-btn" data-act="clearHistPinned" data-sid="' + esc(sid) + '" type="button" title="一键隐藏本区全部条目（✕ 同款，仅显示层）">🧹</button></li>';
              if (pinnedHistOpen[sid]) ph_html += listHtml(pDone);
            }
            if (pOpen_.length) {
              ph_html += '<li class="tfb-histoggle" data-act="toggleHistOpenPinned" data-sid="' + esc(sid) + '" title="展开/收起被抛弃的历史任务">▸ 被抛弃的历史任务 ' + pOpen_.length + ' 条（点击' + (pinnedOpenHistOpen[sid] ? "收起" : "展开") + '）' +
                '<button class="tfb-btn" data-act="clearHistOpenPinned" data-sid="' + esc(sid) + '" type="button" title="一键隐藏本区全部条目（✕ 同款，仅显示层）">🧹</button></li>';
              if (pinnedOpenHistOpen[sid]) ph_html += listHtml(pOpen_);
            }
            html += '<ul class="tfb-pinlist" data-ownsid="' + esc(sid) + '">' + ph_html + "</ul>";
          } else {
    ...
    function buildUI() {
      ...
      root.addEventListener("click", function (e) {
        ...
        if (act === "close") hide();
          ...
          // Plan-A3: expand/collapse the completed-history fold
          historyOpen = !historyOpen;
          render();
        } else if (act === "toggleHistoryOpen") {
          // Plan-A3.1: expand/collapse the non-completed-history fold
          historyOpenOpen = !historyOpenOpen;
          render();
          ...
          var b5 = bySession[normSid(currentSid)];
          var hid5 = hiddenFor(currentSid);
          ...
          if (changed5) { saveHidden(); historyOpen = false; historyOpenOpen = false; }
          render();
          ...
          e.stopPropagation();
          var hid7 = hiddenFor(currentSid);
          var restored7 = false;
          for (var k7 in hid7) { delete hid7[k7]; restored7 = true; }
          if (restored7) { saveHidden(); historyOpen = true; historyOpenOpen = true; }
          render();
        } else if (act === "toggleHistPinned" && sid) {
          // Plan-A8: pinned-session completed-history fold (per-session state)
          e.stopPropagation();
          pinnedHistOpen[sid] = !pinnedHistOpen[sid];
          render();
        } else if (act === "toggleHistOpenPinned" && sid) {
          // Plan-A8: pinned-session abandoned-history fold (per-session state)
          e.stopPropagation();
          pinnedOpenHistOpen[sid] = !pinnedOpenHistOpen[sid];
          render();
        } else if (act === "clearHistPinned" && sid) {
          // Plan-A8: batch-hide the pinned session's COMPLETED history rows
          // (same ✕ semantics: display-only, scoped to that pinned sid).
          e.stopPropagation();
          var hidP = hiddenFor(sid);
          var bP = bySession[normSid(sid)];
          var pLen = (bP && typeof bP.snapLen === "number") ? bP.snapLen : 0;
          var tP = (bP && bP.todos) || [];
          var chP = false;
          for (var ip = pLen; ip < tP.length; ip++) {
            if (tP[ip] && tP[ip].status === "completed" && tP[ip].content && !hidP[tP[ip].content]) {
              hidP[tP[ip].content] = true; chP = true;
            }
          }
          if (chP) { saveHidden(); pinnedHistOpen[sid] = false; }
          render();
        } else if (act === "clearHistOpenPinned" && sid) {
          // Plan-A8: batch-hide the pinned session's ABANDONED history rows
          e.stopPropagation();
          var hidQ = hiddenFor(sid);
          var bQ = bySession[normSid(sid)];
          var qLen = (bQ && typeof bQ.snapLen === "number") ? bQ.snapLen : 0;
          var tQ = (bQ && bQ.todos) || [];
          var chQ = false;
          for (var iq = qLen; iq < tQ.length; iq++) {
            if (tQ[iq] && tQ[iq].status !== "completed" && tQ[iq].content && !hidQ[tQ[iq].content]) {
              hidQ[tQ[iq].content] = true; chQ = true;
            }
          }
          if (chQ) { saveHidden(); pinnedOpenHistOpen[sid] = false; }
          render();
