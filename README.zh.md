...
**让 AI 的任务清单跑出侧栏，常驻在你眼前。**

[English](README.md) | 简体中文

![悬浮球展开面板](docs/images/todo-float-ball-expanded.png)

> 展开态：左上角 `7/8` 胶囊显示总进度；主区按状态列出全部任务（✓ 已完成绿勾 / ▶ 进行中橙脉动），已折叠的历史分组可点击展开，「一键清理全部历史」；底部一行提示「⚠ 还有 1 项未完成」。右侧是「固定会话」区。

![悬浮球收起态](docs/images/todo-float-ball-collapsed.png)

> 收起态：球体显示 `7/8` 进度分数，清单以窄条形态贴着屏幕边缘常驻，不挡内容。
